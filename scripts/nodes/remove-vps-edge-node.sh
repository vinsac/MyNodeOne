...
#
# Usage:
#   sudo ./scripts/nodes/remove-vps-edge-node.sh [NODE_NAME]
###############################################################################
...
if [ -n "$VPS_IP" ]; then
    ...
    if [ -f "$PROJECT_ROOT/scripts/domains/multi-domain-registry.sh" ]; then
        bash "$PROJECT_ROOT/scripts/domains/multi-domain-registry.sh" unregister-vps "$VPS_IP"
    else
