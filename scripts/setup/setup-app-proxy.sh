...
show_usage() {
    cat << 'EOF'
Usage: sudo ./scripts/setup/setup-app-proxy.sh <app-name> <namespace> [options]
...
Examples:
  ...
  sudo ./scripts/setup/setup-app-proxy.sh immich immich

  # Specify custom port:
  sudo ./scripts/setup/setup-app-proxy.sh jellyfin media --proxy-port 8081

  # Custom service name:
  sudo ./scripts/setup/setup-app-proxy.sh nextcloud files --service-name nextcloud
...
echo "  2. Or run: sudo ./scripts/vps/configure-vps-route.sh $APP_NAME 80 <subdomain> <domain>"
echo ""
