# Tool and Component Versions

This document tracks the versions of cluster-critical tools, images, and Helm charts used by MyNodeOne.

## Kubernetes and Cluster Core

- **K3s**: `v1.28.5+k3s1`  
  Defined in `scripts/bootstrap-control-plane.sh` (`K3S_VERSION`).
- **kubectl (Linux)**: `v1.28.5`  
  Installed by `scripts/setup-laptop.sh` using a pinned download URL.

## Helm and Package Management

- **Helm client**: `v3.15.3`  
  Installed by `scripts/setup-laptop.sh` when Helm is not present.

## Observability and Monitoring

- **kube-prometheus-stack (Prometheus + Grafana)**: Helm chart `55.5.0`  
  Installed in `scripts/bootstrap-control-plane.sh`.
- **Loki stack**: Helm chart `2.10.1`  
  Installed in `scripts/bootstrap-control-plane.sh`.
- **Grafana**: Managed via the `kube-prometheus-stack` chart version above.

## Ingress, Networking, and Storage

- **Traefik (Kubernetes Ingress)**: Helm chart `26.0.0`  
  Installed in `scripts/bootstrap-control-plane.sh`.
- **MetalLB**: Helm chart (version pinned in `scripts/bootstrap-control-plane.sh`).
- **Longhorn**: Helm chart `1.5.3`  
  Installed in `scripts/bootstrap-control-plane.sh`.
- **MinIO**: Installed via Helm in `scripts/bootstrap-control-plane.sh` (chart version pinned there).

## GitOps and Platform Services

- **Argo CD**: Helm chart `5.51.6` (primary install)  
  Fallback raw manifests from Argo CD `v2.9.3` are used if Helm installation fails.

## CLI Utilities on Management Laptops

- **k9s (Linux)**: `v0.32.5`  
  Installed by `scripts/setup-laptop.sh` using a pinned GitHub release.

## VPS and Edge Node Components

- **Docker Compose**: `2.24.5`  
  Installed in `scripts/setup-vps-node.sh` via a pinned GitHub release tarball.
- **Traefik (VPS Edge)**: Docker image `traefik:v2.11`  
  Used in `scripts/setup-vps-node.sh` (`docker-compose.yml`).

## Application Templates

- **Node.js base image for app templates**: `node:20-alpine`  
  Used in the Dockerfile generated by `scripts/create-app.sh`.

---

If you update any of the pinned versions in scripts or manifests, please also update this document to keep it in sync.
