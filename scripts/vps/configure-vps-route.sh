...
if [[ -z "$APP_NAME" ]] || [[ -z "$APP_PORT" ]] || [[ -z "$SUBDOMAIN" ]] || [[ -z "$DOMAIN" ]]; then
    cat << 'EOF'
Usage: sudo ./scripts/vps/configure-vps-route.sh <app-name> <port> <subdomain> <domain> [namespace/service]

Example: 
  sudo ./scripts/vps/configure-vps-route.sh immich 3001 photos example.com
  sudo ./scripts/vps/configure-vps-route.sh llm-chat 80 chat example.com llm-chat/open-webui
...
Hint: If your service is in a different namespace, use:
  sudo ./scripts/vps/configure-vps-route.sh $APP_NAME $APP_PORT $SUBDOMAIN $DOMAIN <namespace>/<service-name>"
    fi
fi
...
if [[ -z "${VPS_EDGE_IP:-}" ]]; then
    ...
    if [[ -z "$VPS_EDGE_IP" ]]; then
        ...
        echo "  sudo ./scripts/installation/install-vps-edge-node.sh"
        echo ""
