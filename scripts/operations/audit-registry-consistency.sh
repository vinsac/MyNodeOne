...
if grep -q "Validating registry structure" scripts/setup/setup-vps-node.sh; then
    log_pass "VPS setup includes registry structure validation"
else
    log_warn "VPS setup missing registry structure validation"
fi

if grep -q "has.*domains.*and has.*vps_nodes" scripts/setup/setup-vps-node.sh; then
...
if grep -q "Final SSH Connectivity Check" scripts/setup/setup-vps-node.sh; then
