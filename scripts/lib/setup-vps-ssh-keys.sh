...
###############################################################################
# 3. Copy keys to VPS with retry logic
###############################################################################

# Helper function to check if key is already on VPS
check_key_on_vps() {
    local key_content="$1"
    ...
    if ssh -o ConnectTimeout=5 "$VPS_USER@$VPS_IP" "grep -q '$key_fingerprint' ~/.ssh/authorized_keys 2>/dev/null || ssh-keygen -l -f ~/.ssh/authorized_keys 2>/dev/null | grep -q '$key_fingerprint'"; then
        return 0
    else
        return 1
    fi
...
if check_key_on_vps "$ROOT_PUB_KEY"; then
    echo "[INFO] Root key already exists on VPS, skipping copy"
else
    ...
    while [ $retry_count -lt $MAX_RETRIES ]; do
        ...
        if echo "$ROOT_PUB_KEY" | ssh -o StrictHostKeyChecking=accept-new -o ConnectTimeout=10 "$VPS_USER@$VPS_IP" "mkdir -p ~/.ssh && chmod 700 ~/.ssh && cat >> ~/.ssh/authorized_keys && chmod 600 ~/.ssh/authorized_keys"; then
    ...
    if check_key_on_vps "$ROOT_PUB_KEY"; then
        echo "[SUCCESS] Root key verified on VPS"
    else
...
if [ "$REMOTE_ACTUAL_USER" != "root" ]; then
    ...
    # Check if key already exists on VPS
    if check_key_on_vps "$USER_PUB_KEY"; then
        echo "[INFO] User key already exists on VPS, skipping copy"
    else
        # Try to copy with retries
        retry_count=0
        copy_success=false
        ...
        while [ $retry_count -lt $MAX_RETRIES ]; do
            ...
            if ssh-copy-id -i "$USER_PUB_KEY_PATH" -o StrictHostKeyChecking=accept-new -o ConnectTimeout=10 "$VPS_USER@$VPS_IP" >/dev/null 2>&1; then
        ...
        # Verify key was copied
        if check_key_on_vps "$USER_PUB_KEY"; then
            echo "[SUCCESS] User key verified on VPS"
        else
